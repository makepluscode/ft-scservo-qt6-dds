...
class SCSCL {
public:
  SCSCL(SCSerial *scserial_) : scserial_(scserial_) {
    // scserial_->set_end(1);
  }

  int write_pos(uint8_t ID, uint16_t Position, uint16_t Time,
                uint16_t Speed = 0) // 普通写单个舵机位置指令
  {
    uint8_t bBuf[6];
  ...
  int reg_write_pos(
      uint8_t ID, uint16_t Position, uint16_t Time,
      uint16_t Speed = 0) // 异步写单个舵机位置指令(RegWriteAction生效)
  {
    uint8_t bBuf[6];
    scserial_->host_2_scs(bBuf + 0, bBuf + 1, Position);
    scserial_->host_2_scs(bBuf + 2, bBuf + 3, Time);
  ...
  void sync_write_pos(uint8_t ID[], uint8_t IDN, uint16_t Position[],
                      uint16_t Time[],
                      uint16_t Speed[]) // 同步写多个舵机位置指令
  {
    uint8_t *offbuf = new uint8_t[6 * IDN]; // VLA 대신 동적 할당
    for (uint8_t i = 0; i < IDN; i++) {
      ...
      scserial_->host_2_scs(offbuf + i * 6 + 0, offbuf + i * 6 + 1,
                            Position[i]);
